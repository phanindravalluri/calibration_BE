...
const COOKIE_MAX_AGE = Number(
  process.env.COOKIE_MAX_AGE || 24 * 60 * 60 * 1000
);
...
router.post("/signup", async (req, res) => {
  try {
    const { email, username, password, mobile, company } = req.body;
    ...
    const user = new User({ email, username, passwordHash, mobile, company });
    await user.save();
...
router.post("/login", async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { uid: user._id, role: user.role, username: user.username, company: user.company },
      SESSION_SECRET,
      { expiresIn: "24h" }
    );
